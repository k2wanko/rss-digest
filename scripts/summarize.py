...
SERVICE_PROMPT_TEMPLATE = """あなたはRSSダイジェストの要約アシスタントです。以下は本日「{source}」から収集した記事一覧です。

各記事について日本語で1文(60字程度)の要約を書いてください。raw(RSSの生データ)が短すぎる・意味を成さない場合(例: HackerNewsのメタ情報のみ等)は、linkの記事ページを実際に取得して内容を確認してから要約してください。記事数が多い場合、取得は多くとも3〜5件程度に留め、残りはraw・titleから推測して要約してください。
また、この一覧全体から「{source}」の今日の傾向をまとめた日本語1〜2文のサービス要約も書いてください。
...
DAY_PROMPT_TEMPLATE = """以下は本日のRSSダイジェストを構成する各サービスの要約です。全体を俯瞰した日本語2〜3文の日次要約を書いてください(注目すべき話題やまたがるテーマを中心に)。
