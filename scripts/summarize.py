...
DAY_PROMPT_TEMPLATE = """以下は本日のRSSダイジェストを構成する各サービスの要約です。全体を俯瞰した日本語4〜6文の日次要約を書いてください。技術的なテーマ・傾向・注目すべき変化を、複数のサービスにまたがる形でまとめてください。
テーマを述べるときは抽象的な言葉だけで終わらせず、「AWSでは〜」「Cloudflareでは〜」のように具体的なサービス名とその内容をセットで挙げてください。ただし全サービスを機械的に列挙するのではなく、関連するテーマごとにグルーピングして書いてください。
